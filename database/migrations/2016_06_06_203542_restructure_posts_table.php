...
class RestructurePostsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('posts', function (Blueprint $table) {
            ...
            $table->text('content_html')->after('content');//MarkDown转HTML
            $table->string('page_image')->after('content_html');//文章缩略图（封面）
            $table->string('meta_description')->after('page_image');//文章备注说明
            $table->boolean('is_draft')->after('meta_description');//文章是否为草稿
            $table->string('layout')->after('is_draft')->default('blog.layouts.post');//使用的布局
        });
    }
